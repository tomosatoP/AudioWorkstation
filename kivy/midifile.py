...
def mute_rules(**kwargs) -> str:
    ...
    for chan in list(kwargs):
        if kwargs[chan]:
            ...
            rules[comment] = {'type': FLUID_MIDI_ROUTER_RULE_TYPE.NOTE,
                              'chan': None, 'param1': None, 'param2': None}
            rules[comment]['chan'] = {'min': int(
                chan), 'max': int(chan), 'mul': 1.0, 'add': 0}
    ...
    # CC: without change
    rules['CC'] = {'type': FLUID_MIDI_ROUTER_RULE_TYPE.CC,
                   'chan': None, 'param1': None, 'param2': None}
    # PROG_CHANGER: without change
    rules['PROG_CHANGER'] = {
        'type': FLUID_MIDI_ROUTER_RULE_TYPE.PROG_CHANGER, 'chan': None, 'param1': None, 'param2': None}
    # PITCH_BEND: without change
    rules['PITCH_BEND'] = {'type': FLUID_MIDI_ROUTER_RULE_TYPE.PITCH_BEND,
                           'chan': None, 'param1': None, 'param2': None}
    # CHANNEL_PRESSURE: without change
    rules['CHANNEL_PRESSURE'] = {
        'type': FLUID_MIDI_ROUTER_RULE_TYPE.CHANNEL_PRESSURE, 'chan': None, 'param1': None, 'param2': None}
    # KEY_PRESSURE: without change
    rules['KEY_PRESSURE'] = {
        'type': FLUID_MIDI_ROUTER_RULE_TYPE.KEY_PRESSURE, 'chan': None, 'param1': None, 'param2': None}
